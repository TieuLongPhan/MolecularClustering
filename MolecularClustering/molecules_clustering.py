...
class Butina_clustering:
    ...
    def __init__(self, df,ID, smiles_col, active_col, mol_col = 'ROMol', activity_thresh = 7, 
                 radius= 2, nBits = 2048, dis_cutoff = 0.7, cps = 5):
        self.data = df
        self.ID = ID
    ...
    def create_cps(self, df):
        ...
        fingerprints = [AllChem.GetMorganFingerprintAsBitVect(mol, radius=self.radius, nBits = self.nBits) for mol in compounds]
